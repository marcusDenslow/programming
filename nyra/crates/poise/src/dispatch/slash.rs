...
fn find_matching_command<'a, 'b, U, E>(
  ...
  commands.iter().find_map(|cmd| {
    if interaction_name != cmd.name && Some(interaction_name) != cmd.context_menu_name.as_deref() {
      return None;
    }
    ...
    if let Some((sub_name, sub_interaction)) =
      interaction_options
        ...
        .find_map(|option| match &option.value {
          serenity::ResolvedValue::SubCommand(o) | serenity::ResolvedValue::SubCommandGroup(o) => {
            Some((&option.name, o))
          }
          _ => None,
        })
