...
pub struct CommandParameter<U, E> {
  ...
  pub autocomplete_callback: Option<
    for<'a> fn(
      ...
      -> BoxFuture<'a, Result<serenity::CreateAutocompleteResponse, crate::SlashArgError>>,
  >,
  #[doc(hidden)]
  pub __non_exhaustive: (),
}
...
impl<U, E> CommandParameter<U, E> {
  ...
  pub fn create_as_slash_command_option(&self) -> Option<serenity::CreateCommandOption> {
    ...
    for (i, choice) in self.choices.iter().enumerate() {
      builder = builder.add_int_choice_localized(&choice.name, i as _, choice.localizations.iter());
    }
